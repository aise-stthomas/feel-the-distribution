...
Every call is appended to runs/distribution.jsonl as it happens, so a run that hits
the rate limit at call 63 has still given you 63 samples.
"""
...
def collect(n: int, temperatures: list, model: str, provider: str, out: Path) -> list[dict]:
    ...
    for temperature in temperatures:
        ...
        print(f"\n{label}: {n} calls to {model} via {provider}", flush=True)
        seen = Counter()
        for i in range(1, n + 1):
            ...
            seen[(rec["action"], rec["refund_amount"])] += 1
            if i % 10 == 0 or i == n:
                print(f"  {i:3d}/{n}  distinct so far: {len(seen):2d}  "
                      f"latest: {rec['action']} {rec['refund_amount']}", flush=True)
    return records
...
def summarize(records: list[dict]) -> None:
    ...
    print("  1. How many distinct outputs at temperature 0?")
